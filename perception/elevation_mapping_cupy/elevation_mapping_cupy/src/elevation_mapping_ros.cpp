...
namespace elevation_mapping_cupy
{
        ...
        isGridmapUpdated_(false)
  {
    ...
    // mode message input
    nh.param<bool>("mode/mode_activate", mode_activate_, false);
    nh.param<std::string>("mode/mode_topic", mode_topic_, "/mode");
    nh.param<std::string>("mode/set_mode_name", set_mode_name_, "mode_default_name");
    ROS_INFO_STREAM("mode_activate: " << mode_activate_ << ", mode_topic: " << mode_topic_ << ", set_mode_name: " << set_mode_name_);

    enablePointCloudPublishing_ = enablePointCloudPublishing;

    // Iterate all the subscribers
    // setup mode subscribers
    if (mode_activate_)
    {
      modeSub_ = nh.subscribe(mode_topic_, 1, &ElevationMappingNode::ModeCallback, this);
    }
  ...
  void ElevationMappingNode::pointcloudCallback(const sensor_msgs::PointCloud2 &cloud, const std::string &key)
  {
    ...
    if (mode_activate_)
    {
      if (mode_correct_ == false)
      {
        // ROS_WARN("Waiting for correct mode");
        return;
      }
    }
                                                  ...
                                                  const elevation_map_msgs::ChannelInfoConstPtr &channel_info_msg)
  {
    ...
    if (mode_activate_)
    {
      if (mode_correct_ == false)
      {
        //ROS_WARN_THROTTLE(5.0, "ElevationMapping: Waiting for correct mode (expected: %s)", set_mode_name_.c_str());
        return;
      }
    }
  ...
  void ElevationMappingNode::ModeCallback(const std_msgs::String::ConstPtr &msg)
  {
    std::string mode = msg->data;
    // check whether input mode message corresponds to the right mode
    if (mode != set_mode_name_)
    {
      mode_correct_ = false;
      //ROS_WARN_THROTTLE(5.0, "ElevationMapping: Received mode '%s', but expected '%s'", mode.c_str(), set_mode_name_.c_str());
    }
    else
    {
      mode_correct_ = true;
      ROS_INFO("ElevationMapping: Mode matched, start processing");
    }
  }
